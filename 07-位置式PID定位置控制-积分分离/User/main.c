...
void TIM1_UP_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) == SET)
	{
		...
		if (Count >= 40)		//如果计次40次，则if成立，即if每隔40ms进一次
		{
			...
			/*误差积分+积分分离*/
			// fabs()计算浮点数的绝对值
			// 积分分离的思路:既然远离目标时不需要 I 项，那就别让它工作；只有接近目标时才启用它
			// 误差大（远离目标）→ 关闭积分，清零 ErrorInt → 避免积累"历史债务" → 防超调
			// 误差小（接近目标）→ 开启积分，正常累加   → 利用积分突破摩擦力 → 消稳态误差
			if (fabs(Error0) < 50)		//如果当前误差值小于指定的阈值(此时说明接近目标了)
			{
				ErrorInt += Error0;		//才进行正常的误差积分(开启积分，消除稳态误差)
			}
			else						//否则，即当前误差值过大(此时说明远离目标)
			{
				ErrorInt = 0;			//此时不进行积分，同时把误差积分直接清零(关闭积分，防止超调)
			}
