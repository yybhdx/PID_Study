...
void Mode11_Loop(void)
{
	...
	if (Key_Check(2, KEY_CLICK))	//K2按下，K1所在位置反色闪烁一次
	{
	...
	if (Key_Check(3, KEY_CLICK))	//K3按下，K1所在位置反色闪烁一次
	{
...
{
	if (Key_Check(1, KEY_CLICK))	//K1按下，PWM_Duty加10
	{
		PWM_Duty += 10;
	}
	if (Key_Check(2, KEY_CLICK))	//K2按下，PWM_Duty减10
	{
		PWM_Duty -= 10;
	}
	...
	Motor_SetPWM(2, PWM_Duty);		//把PWM_Duty变量的值赋值给右侧两个电机接口
