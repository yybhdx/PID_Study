...
/*定义变量*/
float Target, Actual, Out;			//目标位置值，实际位置值，输出控制值
float Kp, Ki, Kd;					//比例项，积分项，微分项系数
float Error0, Error1, ErrorInt;		//本次误差，上次误差，误差积分
float DifOut;						//微分项输出（滤波后）
...
//		}
		...
		Serial_Printf("%f,%f,%f,%f\r\n", Target, Actual, Out, DifOut);		//串口打印目标值、实际值和输出值
...
void TIM1_UP_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM1, TIM_IT_Update) == SET)
	{
		...
		if (Count >= 40)		//如果计次40次，则if成立，即if每隔40ms进一次
		{
			...
			Actual += rand() % 41 - 20; // 引入 -20 到 +20 的随机噪声，模拟实际系统的高频干扰，验证不完全微分的抗噪效果
			...
			/* 微分项计算（不完全微分 / 一阶低通滤波） */
			/* 纯微分：DifOut = Kd * (Error0 - Error1); 容易放大高频噪声 */
			/* 不完全微分：新的DifOut = (1-a) * 本次原始微分 + a * 上次微分输出 */
			/* a 为滤波系数(0~1)，a 越大滤波越平滑，但响应延迟也越大 */
			float a = 0.9;
